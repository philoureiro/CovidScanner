...
import mlp_classifier, rf_classifier, svm_classifier, ridge_classifier, sgd_classifier

def main():
    mainStartTime = time.time()
    results = []
    modelNames = ['MLP', 'SVM', 'RF', 'Ridge', 'SGD']
    print(f'[INFO] *********MLP**********.')
    ...
    results.append(result)
    print(f'[INFO] *********Ridge**********.')
    result, featureName = ridge_classifier.main()
    results.append(result)
    print(f'[INFO] *********SGD**********.')
    result, featureName = sgd_classifier.main()
    results.append(result)
...
def plotResults(modelNames, results, featureName):
    fig, ax = plt.subplots()
    bar_container = ax.bar(modelNames, results, color=['red', 'green', 'blue', 'cyan', 'magenta', 'orange'])
